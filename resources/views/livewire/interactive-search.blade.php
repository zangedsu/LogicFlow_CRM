...
<div class="relative flex flex-1" x-data="{ open: false }" @click.away="open = false" @close.stop="open = false" @keyup.escape="open = false">
    <form class="w-full"  action="#" method="GET">
        ...
        <input @click="open = true" wire:model.live.debounce="searchText" wire:change.self="search" id="search-field" class="block h-full w-full border-0 bg-transparent py-0 pr-0 pl-8 placeholder:text-gray-400 text-gray-900 focus:ring-0 dark:text-gray-200 sm:text-sm" placeholder="Искать..." type="search" name="search">
    </form>
        ...
        <div x-show="open"
             ...
             class="absolute top-16 left-0 z-50 mt-2 w-full rounded-md shadow-lg"
             style="display: none;">
            <div class="rounded-md z-30 ring-1 ring-black ring-opacity-5 bg-zinc-900 backdrop-blur-xl min-h-28 scroll-auto max-h-screen">
                ...
                @if($searchText != '' && $searchText != '?' && count($actions) == 0 && count($clients) == 0  && count($projects) == 0  && count($tasks) == 0)
                    <!-- Empty state -->
...
{{--                            </li>--}}
                    ...
                    <li class="p-2">
                        ...
                        <ul class="text-sm max-h-32 overflow-y-auto scroll-auto text-gray-400">
                            <!-- Active: "bg-gray-800 text-white" -->
                            @foreach($clients as $client)
                                <li wire:key="{{uuid_create()}}" class="flex cursor-default select-none items-center rounded-md px-3 py-2 group">
                    ...
                    @if($tasks)
                            <li class="p-2">
                                <div class="flex py-4">
                                    <svg  class="h-6 w-6 flex-none text-gray-500" fill="currentColor"  viewBox="0 0 24 24">
                                        <path d="m10 15.586-3.293-3.293-1.414 1.414L10 18.414l9.707-9.707-1.414-1.414z"></path>
                                    </svg>
                                    <h2 class="ml-2 font-bold dark:text-gray-200">Задачи</h2>
                                </div>
                                <ul class="text-sm max-h-32 overflow-y-auto scroll-auto text-gray-400">
                                    <!-- Active: "bg-gray-800 text-white" -->
                                    @foreach($tasks as $task)
                                        <li wire:key="{{uuid_create()}}" class="flex cursor-default select-none items-center hover:border rounded-md px-3 py-2 group">
                                            <!-- Active: "text-white", Not Active: "text-gray-500" -->
                                            <span class="ml-3 flex-auto truncate">{{$task['name']}}</span>
                                            <span class="ml-3 flex-none text-xs font-semibold text-gray-400"><kbd class="font-sans">></kbd></span>
                                        </li>
                                    @endforeach
