    ...
    <ul role="list" class="mt-6 space-y-6 max-h-96 overflow-y-auto scroll-auto -scroll-ml-10">
        @foreach($comments as $comment)
            <li class="relative flex gap-x-4">
                ...
                <img src="{{ $comment->user()->first()->profile_photo_path ? asset('storage/'. $comment->user()->first()->profile_photo_path) :$comment->user()->first()->profile_photo_url }}" alt="" class="relative mt-3 h-6 w-6 flex-none rounded-full bg-gray-50">
                <div class="flex-auto bg-zinc-900 rounded-md p-3 ring-1 ring-inset ring-gray-200">
                    <div class="flex justify-between gap-x-4">
                        <div class="py-0.5 text-xs leading-5 text-gray-50"><span class="font-medium text-gray-100">{{$comment->user()->first()->name}}</span> написал(а)</div>
                        <time datetime="2023-01-23T15:56" class="flex-none py-0.5 text-xs leading-5 text-gray-500">{{$this->getFormattedTime($comment->created_at)}}</time>
                    </div>
