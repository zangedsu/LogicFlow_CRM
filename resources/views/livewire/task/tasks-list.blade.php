                            ...
                            <div class="mt-1 flex items-center gap-x-1.5">
                                ...
                                <x-dropdown>
                                    ...
                                    <x-slot:content>
                                        ...
                                        @if(Auth::user()->hasTeamPermission(Auth::user()->currentTeam()->first(), 'edit'))
                                            <x-dropdown-link href="{{route('tasks.edit', $task->id)}}">
                                                Редактировать
                                            </x-dropdown-link>
                                        @endif
                                        @if(Auth::user()->hasTeamPermission(Auth::user()->currentTeam()->first(), 'delete'))
                                        <x-dropdown-link disabled wire:confirm="Вы уверены, что хотите удалить эту задачу?" wire:click="deleteTask({{$task->id}})">
                                            Удалить
                                        </x-dropdown-link>
                                        @endif
                                    </x-slot:content>
